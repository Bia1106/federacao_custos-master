...
import { DinamicaDespesa } from 'src/services/metabase';
...
const RtpDinamicaDespesa: React.FC = () => {
  ...
  const hideParametres =
    '#hide_parameters=phospital,pusuario,ano,pcentrocusto,ptiporecurso';
  const linkAno = ItemAno.idAno === 0 ? '' : `ano=${ItemAno?.idAno}`;

  const linkCCHospital =
    itemcAno.idAno === 0 ? '' : `ano=${itemcAno?.idAno.toString()}`;

  const handleCarregaLink = async () => {
    const link = `${DinamicaDespesa}?${linkHospital}&${linkAno}&${linkUsuario}&${linkCCHospital}${hideParametres}`;
...
export default RtpDinamicaDespesa;
